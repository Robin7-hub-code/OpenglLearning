...
#include"../material/grassPhongInstanceMaterial.h"
...
Renderer::Renderer() {
	...
	mGrassPhongInstanceShader = new Shader("assets/shaders/grassphongInstance.vert", "assets/shaders/grassphongInstance.frag");
}
...
Shader* Renderer::pickShader(MaterialType type) {
	...
	case MaterialType::PhongInstanceMaterial:
		...
		break;
	case MaterialType::GrassPhongInstanceMaterial:
		result = mGrassPhongInstanceShader;
		break;
	default:
...
void Renderer::renderObject(
	...
	if (object->getType() == ObjectType::Mesh||object->getType()==ObjectType::InstanceMesh) {
		...
		case MaterialType::PhongInstanceMaterial:
		{
			...
			InstanceMesh* im = (InstanceMesh*)mesh;
			//diffuse贴图帧更新
			//将纹理采样器与纹理单元进行挂钩
			shader->setInt("sampler", 0);
			//将纹理与纹理单元进行挂钩
			...
			//透明度
			shader->setFloat("opacity", material->mOpacity);
			/*shader->setMatrixArray4x4("matrices", im->mInstanceMats, im->mInstanceCount);*/
		}
										break;
		case MaterialType::GrassPhongInstanceMaterial:
		{
			GrassPhongInstanceMaterial* phongMat = (GrassPhongInstanceMaterial*)material;
			InstanceMesh* im = (InstanceMesh*)mesh;
			...
			shader->setFloat("uvScale", phongMat->mUVScale);
			shader->setFloat("brightness", phongMat->mBrightNess);
			shader->setFloat("time",glfwGetTime());

			shader->setFloat("windScale", phongMat->mWindScale);
			shader->setVector3("windDir", phongMat->mWindDir);
			shader->setFloat("phiScale", phongMat->mPhiScale);

			shader->setInt("cloudMask", 2);
			phongMat->mCloudMask->bind();

			shader->setVector3("cloudBlack", phongMat->mCloudBlackColor);
			shader->setVector3("cloudWhite", phongMat->mCloudWhiteColor);

			shader->setFloat("cloudUvScale", phongMat->mCloudUvScale);
			shader->setFloat("CloudSpeed", phongMat->mCloudSpeed);
			shader->setFloat("lerp_para", phongMat->mLerp_para);
		}
										break;
		default:
			break;
