...
// 将输入的顶点位置，转化为齐次坐标（3维-4维）
	...
	vec3 windDirN=normalize(windDir);
	float phi=dot(windDirN,transformPosition.xyz);
	transformPosition+=vec4(sin(time+phi/phiScale)*(1-aColor.r)*windScale*windDirN,0.0);
